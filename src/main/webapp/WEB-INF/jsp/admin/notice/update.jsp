...
<body>
			...
			<div class="main-content">
			    <div class="container-fluid">
			        	...
			        	%>
							<div class="card">
							    <div class="content" id="<%=v %>">
						        	<input id="fileName_<%=v %>" name="fileName" type="hidden" value="<%=bc.getTumbnailPath() %>">
						        	
						            <div class="form-group">
						                <label class="col-md-2 control-label">TITLE (<%=v %>)</label>
						                <div class="col-md-10">
						                    <input name="subject" type="text" placeholder="title" class="form-control" value="<%=bc.getSubject() %>">
						                </div>
						            </div>
						            <div class="form-group">
						            ...
						            <div class="form-group">
						            	<div class="col-md-12">
											<textarea name="content" id="ir1_<%=v %>" rows="10" cols="100" style="width:100%; height:412px; display:none;"><%=bc.getContent() %></textarea>
						            	</div>
						            </div>
							    </div>
							</div>
					...
					<div class="pull-right">
						...
						<button type="button" onclick="articleDeleteSubmit();" class="btn btn-danger btn-fill btn-wd">DELETE</button>
	...
	<c:url var="deleteURL"  value="/admin/notice/delete" />
	<form:form name="deleteForm" method="post" action="${deleteURL }">
		<input name="num" type="hidden" value="<%=request.getParameter("num") %>">
	</form:form>
	...
	nhn.husky.EZCreator.createInIFrame({
		...
		fOnAppLoad : function(){
			...
			for (BoardContent bc : boardContent) {
				if(v.equals(bc.getVersion())){
			%>
			oEditors.getById["ir1_<%=v %>"].exec("LOAD_CONTENTS_FIELD");
			<%
				}
			}
			%>
